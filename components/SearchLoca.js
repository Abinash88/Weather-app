      ...
      <div className="flex w-[100%] md:w-[70%] items-center flex-col mt-3 justify-center relative ">
        ...
        >
          {DisplayItems?.length > 0 ? DisplayItems?.map((item, index) => {
            return <div className="w-full py-1 cursor-pointer hover:bg-gray-200 mb-1 px-3 bg-gray-100">
              <div key={index}  onClick={(e) => GetData(e)} className="">{item}</div>
            </div>
          }) : <div className="w-full h-full text-gray-600 font-bold flex items-center justify-center ">No Data Found!</div>}
        </div>
      </div>
    </div>
  );
